...
func commandCatch(pokemon string, cfg *config, cache *pokecache.Cache, pokedex *map[string]Pokemon) error {
	if pokemon == "" {
		fmt.Println("specify a pokemon!")
		return nil
	}
	...
	if hit {
		...
		if responseStream.StatusCode > 299 {
			fmt.Printf("there's no such pokemon as %s!\n", pokemon)
			return nil
		}
	...
	if rand.Intn(500) < responseStruct.BaseExperience {
		fmt.Printf("%s escaped!\n", pokemon)
		return nil
	}

	fmt.Printf("%s was caught!\n", pokemon)
	(*pokedex)[pokemon] = responseStruct
	return nil
}
